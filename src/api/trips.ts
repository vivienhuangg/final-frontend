...
export async function createTrip(title: string, startDate: string, endDate: string): Promise<{ trip: string }> {
  const session = getSession();
  const { data } = await http.post('/trips/new', { session, title, startDate, endDate });
  if (data && typeof data === 'object' && 'error' in data && (data as any).error) throw new Error((data as any).error);
  return data as { trip: string };
}

export async function modifyTrip(trip: string, title: string, startDate: string, endDate: string): Promise<{ trip: string }> {
  const session = getSession();
  const { data } = await http.post('/trips/modify', { session, trip, title, startDate, endDate });
  if (data && typeof data === 'object' && 'error' in data && (data as any).error) throw new Error((data as any).error);
  return data as { trip: string };
}
